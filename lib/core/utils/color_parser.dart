...
class ColorParser {
  ...
  static Color? parse(String value) {
    ...
    final hsvMatch = RegExp(
      r'^(?:hsv|hsb)?\(?\s*([\d.]+)(?:deg|°)?\s*,\s*([\d.]+)%?\s*,\s*([\d.]+)%?(?:\s*,\s*([\d.]+))?\s*\)?$',
      caseSensitive: false,
    ).firstMatch(value);
    if (hsvMatch != null) {
    ...
    final labMatch = RegExp(
      r'^(?:lab)?\(?\s*L\*\s*([\d.]+)\s*,\s*a\*\s*(-?[\d.]+)\s*,\s*b\*\s*(-?[\d.]+)\s*\)?$',
      caseSensitive: false,
    ).firstMatch(value);
    if (labMatch != null) {
    ...
    final labMatch2 = RegExp(
      r'^lab\(\s*([\d.]+)\s*,\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)\s*\)$',
      caseSensitive: false,
    ).firstMatch(value);
    if (labMatch2 != null) {
    ...
    final xyzMatch = RegExp(
      r'^(?:xyz)?\(?\s*X:\s*([\d.]+)\s*,\s*Y:\s*([\d.]+)\s*,\s*Z:\s*([\d.]+)\s*\)?$',
      caseSensitive: false,
    ).firstMatch(value);
    if (xyzMatch != null) {
    ...
    final xyzMatch2 = RegExp(
      r'^xyz\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\s*\)$',
      caseSensitive: false,
    ).firstMatch(value);
    if (xyzMatch2 != null) {
    ...
    // 8. HWB (hwb(29, 10%, 20%))
    final hwbMatch = RegExp(
      r'^hwb\(\s*([\d.]+)(?:deg|°)?\s*,\s*([\d.]+)%?\s*,\s*([\d.]+)%?(?:\s*,\s*([\d.]+))?\s*\)$',
      caseSensitive: false,
    ).firstMatch(value);
    if (hwbMatch != null) {
